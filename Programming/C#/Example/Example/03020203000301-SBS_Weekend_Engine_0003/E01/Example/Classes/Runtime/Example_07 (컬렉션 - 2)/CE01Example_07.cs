 ...
 * 
 * 셋이란?
 * - 데이터의 중복을 허용하지 않는 컬렉션을 의미한다. (즉, 해당
 * 컬렉션은 수학의 집합과 같은 특징을 지니고 있다는 것을 알 수 있다.)
 * 
 * 딕셔너리란?
 * - 데이터를 키와 벨류의 쌍으로 관리하는 컬렉션을 의미한다. (즉,
 * 딕셔너리는 탐색에 특화 된 컬렉션이라는 것을 알 수 있다.)
 * 
 * 키 vs 벨류
 * - 키는 데이터를 탐색하기 위한 식별자의 역할을 하기 때문에 중복을
 * 허용하지 않는 특징이 존재한다. (즉, 딕셔너리에 동일한 키를 사용
 * 할 경우 내부적으로 문제가 될 수 있다는 것을 알 수 있다.)
 * 
 * 반면, 벨류는 실제 관리되는 데이터를 의미하기 때문에 중복이
 * 가능하다는 차이점이 존재한다. (즉, 딕셔너리는 키를 통해서 사용자가
 * 원하는 벨류를 탐색하는 컬렉션이라는 것을 알 수 있다.)
 */
namespace Example._03020203000301_SBS_Weekend_Engine_0003.E01.Example.Classes.Runtime.Example_07
{
